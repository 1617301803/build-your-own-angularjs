...
function markConstantAndWatchExpressions(ast) {
    ...
    switch (ast.type) {
        ...
        case AST.MemberExpression:
            markConstantAndWatchExpressions(ast.object); if (ast.computed) {
...
Lexer.prototype.peek = function (n = 1) {
...
AST.prototype.logicalOR = function () {
    ...
    while ((token = this.expect('||'))) {
        left = {
            type: AST.LogicalExpression, left: left,
            operator: token.text, right: this.logicalAND()
        };
    }
    return left;
};
...
AST.prototype.logicalAND = function () {
    ...
    while ((token = this.expect('&&'))) {
        left = {
            type: AST.LogicalExpression, left: left,
            operator: token.text, right: this.equality()
        };
    }
    return left;
};
...
AST.prototype.ternary = function () {
    ...
    if (this.expect('?')) {
        ...
        if (this.consume(':')) {
            var alternate = this.assignment(); return {
...
ASTCompiler.prototype.compile = function (text) {
    ...
    this.stage = 'inputs';
    _.forEach(getInputs(ast.body), (input, idx) => {
        var inputKey = 'fn' + idx;
...
ASTCompiler.prototype.recurse = function (ast, context, create) {
    ...
    switch (ast.type) {
        ...
        case AST.Identifier:
            ensureSafeMemberName(ast.name); intoId = this.nextId();
            var localsCheck;
